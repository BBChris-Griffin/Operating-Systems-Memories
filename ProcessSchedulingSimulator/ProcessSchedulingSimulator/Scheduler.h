...
///////////////////////////////////////////////////////////
// 
// This class calls and controls the scheuding algorithms in the system
//
////////////////////////////////////////////////////////////
#include "stdafx.h"
#include "newTask.h"
...
private:
	...
	FINISHED
	*/
	// Print process start time and end time?
		...
		while (true)
		{
			...
			if (clock >= clock_start_time)
			{
				...
				if (!execHold->IsEmpty())
				{
					if (process_alg == 2)
					{
						execHold->RoundRobinExecution(readyQueue, finishQueue, blockQueue, currentMem, currentIO,
							timeQuantum, clock, outF);
					}
					else
					{
				...
				jobQueue->LeaveJobQueue(readyQueue, waitQueue, finishQueue,rejectedQueue, clock, mem_units, 
					currentMem, io_devices, currentIO, timeQuantum, outF);
